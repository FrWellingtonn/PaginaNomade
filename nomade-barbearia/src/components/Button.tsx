...
const Button: React.FC<ButtonProps> = ({
  ...
  if (href) {
    return (
      <a href={href} className={combinedClasses} onClick={onClick as (e: React.MouseEvent<HTMLAnchorElement>) => void}>
        {children}
      </a>
    );
  }
  ...
  return (
    <button type={type} className={combinedClasses} onClick={onClick as () => void}>
      {children}
    </button>
  );
};
