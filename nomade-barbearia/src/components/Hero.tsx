...
import imgnavalha from '../imgnavalha.jpg';

interface HeroProps {
  onNavClick: (e: React.MouseEvent<HTMLButtonElement>, id: string) => void;
...
const Hero: React.FC<HeroProps> = ({ onNavClick, onWhatsAppClick }) => {
  return (
    <section
      ...
      style={{
        backgroundImage: `linear-gradient(135deg, rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.6)), url(${imgnavalha})`,
      }}
    >
      <div className="max-w-7xl mx-auto px-8 md:px-4">
        <div className="max-w-[600px]">
          ...
          <div className="flex gap-6 flex-wrap md:flex-col md:items-start lg:flex-row lg:justify-center lg:text-center">
            ...
            <Button onClick={(e) => onNavClick(e as any, 'servicos')} variant="outline">
              Nossos Serviços
            </Button>
          </div>
        </div>
